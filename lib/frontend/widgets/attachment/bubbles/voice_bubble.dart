...
class _WaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final n = math.min(amps.length, math.max(12, (size.width / 4).floor()));
    var maxAmp = 1;
    for (final a in amps) {
      if (a > maxAmp) maxAmp = a;
    }
    final slot = size.width / n;
    final barW = math.min(2.4, slot * 0.52);
    final paint = Paint()..strokeCap = StrokeCap.round;

    for (var i = 0; i < n; i++) {
      final from = (i * amps.length / n).floor();
      final to = math.max(from + 1, ((i + 1) * amps.length / n).ceil());
      var total = 0.0;
      for (var sample = from; sample < math.min(to, amps.length); sample++) {
        total += amps[sample];
      }
      final average = total / math.max(1, math.min(to, amps.length) - from);
      final normalized = math.sqrt((average / maxAmp).clamp(0.0, 1.0));
      final h = (4 + normalized * (size.height - 4)).clamp(4.0, size.height);
